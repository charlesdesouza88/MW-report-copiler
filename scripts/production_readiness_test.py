...
def run_live(base: str) -> int:
    ...
    email = (os.environ.get('SUPERADMIN_EMAIL') or creds.get('SUPERADMIN_EMAIL', '')).strip()
    password = (
    ...
    try:
        ...
        body = db.read().decode()
        local_csv_ok = (
            urllib.parse.urlparse(base).hostname in {'127.0.0.1', 'localhost'}
            and ('"mode": "csv"' in body or '"mode":"csv"' in body)
        )
        runner.ok(
            '/health/db',
            '"connected": true' in body or '"connected":true' in body or local_csv_ok,
            body[:80],
        )
    except Exception as exc:
