...
LESSONS_CSV = (
    ...
    'FLOW_TEST,2,15/02/2026,Lesson 2,,\n'
)
...
def run_live(base: str):
    ...
    bad_resp = post('/students/new', {
        ...
        'turma': '',
        'teacher': 'Chuck',
        'csrf_token': csrf_from_html(new_student_html),
    })
    ...
    new_name = f'Live Flow Kid {uuid4().hex[:8]}'
    student_data = _student_form(new_name, 'MASTER')
    student_data['turma_display'] = 'Masters'
    student_data['csrf_token'] = csrf_from_html(bad_body) or csrf_from_html(new_student_html)
    try:
