...
def run_live(base: str):
    ...
    new_name = f'Live Flow Kid {int(time.time())}'
    create_html = get('/students/new?teacher=Amanda').read().decode('utf-8', errors='replace')
    try:
        create_data = _student_form(new_name, 'COMET', teacher='Amanda')
        create_data['csrf_token'] = csrf_from_html(create_html)
