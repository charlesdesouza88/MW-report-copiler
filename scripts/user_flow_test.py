...
LESSONS_CSV = (
    ...
    'FLOW_TEST,1,01/02/2026,Flow test lesson,,\n'
)
...
def run_live(base: str):
    ...
    try:
        html = get('/students/new').read().decode('utf-8', errors='replace')
        form = _student_form(new_name, 'LIVE_FLOW')
        form['csrf_token'] = csrf_from_html(html)
        post('/students/new', form, allow_redirect=False)
        create_ok = False
        loc = ''
